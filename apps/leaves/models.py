...
class LeaveType(CompanyScopedModel):
    ...
    # ق-169: **أساس الأيام** (قرار جواد) — تختاره الشركة.
    #
    # ⚠️ **فالاستحقاق اليوميّ يقسم عليه**: ٣٦٥ للتقويم، أو عدد
    # أيام العمل في السنة لمن يحسب بها.
    accrual_basis_days = models.PositiveSmallIntegerField(
        _("أساس الأيام للاستحقاق"), default=365,
        help_text=_("٣٦٥ للتقويم · أو أيام العمل السنوية"))

    # ق-169: ⚠️⚠️ **ومدّة الانتظار قبل الاستهلاك** (قرار جواد).
    #
    # **فالرصيد يُستحقّ من اليوم الأول، والاستهلاك قد يُمنع** حتى
    ...
    waiting_period_days = models.PositiveSmallIntegerField(
        _("مدّة الانتظار قبل الاستهلاك (يومًا)"), default=0,
        help_text=_("٠ = من أول يوم"))

    # ق-169: ⚠️ **أيُخصَّص رصيده لكل موظف؟** (قرار جواد).
    #
    # **فالمرضية والخاصة تُستحقّ بواقعتها لا برصيد** — وإظهارها
    # في عقد الموظف **يوحي بأنها واجبٌ على الشركة**.
    is_contractual = models.BooleanField(
        _("رصيدٌ تعاقديّ"), default=False,
        help_text=_("يظهر في عقد الموظف ويُخصَّص له"))
...
class ApprovalStep(models.Model):
    ...
    # ق-87: مهلة الدرجة — بعدها ينتقل الطلب للتالية.
    # صفر يعني بلا تصعيد، والقيمة يحدّدها مدير الموارد لكل درجة:
    # اعتماد المشرف يوم، والمدير العام ثلاثة.
    escalate_after_hours = models.PositiveIntegerField(
        _("التصعيد بعد (ساعة)"), default=0,
        help_text=_("صفر = لا تصعيد. ولا يُتجاوز مدير الموارد ولا "
                    "المدير العام ولا آخر درجة"))
