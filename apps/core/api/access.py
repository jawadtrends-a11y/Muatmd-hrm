...
def member_permissions(request, employment_id):
    ...
    if request.method == "PUT":
        ...
        # قائمة فارغة تنزع كل ما يمنحه الدور — وذلك قرار جسيم
        # يقع بالخطأ: من يفتح الشاشة ويحفظ قبل أن تُحمَّل المفاتيح
        # يمحو صلاحيات المستخدم كلها فلا يرى شيئًا في النظام.
        #
        # فمن يقصده يؤكّده صراحةً، ومن أخطأ يُنقَذ.
        if from_role and not wanted and not request.data.get(
                "confirm_revoke_all"):
            return Response({
                "detail": "هذا يُلغي كل صلاحيات المستخدم فلا يرى "
                          "شيئًا في النظام — أكّد إن كنت تقصده",
                "code": "revoke_all_needs_confirm",
                "would_revoke": len(from_role),
            }, status=409)
