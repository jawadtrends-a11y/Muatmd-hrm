...
urlpatterns = [
    ...
    path("api/employees/invite-bulk/",
         accounts_invites_api.invite_bulk, name="invite-bulk"),
    # ومفتوحان: الموظف يفتح الرابط قبل أن يملك حسابًا
    path("api/join/<str:token>/",
    ...
    # ق-140: سلسلة موافقات المسير
    path("api/payroll/approval-steps/", recurring_api.approval_steps,
         name="payroll-approval-steps"),
    path("api/payroll/approval-steps/<int:step_id>/",
         recurring_api.approval_step_detail,
         name="payroll-approval-step-detail"),
    path("api/payroll/runs/<int:run_id>/chain/", recurring_api.run_chain,
         name="payroll-run-chain"),
    ...
    path("api/reports/builder/sources/", builder_api.builder_sources,
         name="builder-sources"),
    path("api/reports/custom/", builder_api.custom_reports,
         name="custom-reports"),
    path("api/reports/custom/<int:report_id>/",
         builder_api.custom_report_detail, name="custom-report-detail"),
    path("api/reports/custom/<int:report_id>/run/", builder_api.run_custom,
         name="run-custom-report"),
    path("api/reports/custom/preview/",
         lambda r: builder_api.run_custom(r, 0), name="preview-custom"),
    ...
    path("api/me/activities/", activities_api.my_activities,
         name="my-activities"),
    path("api/activities/<int:activity_id>/report/",
         activities_api.report_activity, name="report-activity"),
    path("api/activities/<int:activity_id>/review/",
         activities_api.review_activity, name="review-activity"),
